...
/**
  ...
  * <br>
  * Dataset上面的操作为transformations and actions，Transformation转换为新的DataSet，action将会触发计算返回结果
  *
  * <br><br>
  *
  ...
  * <br>DataSet也是延迟计算的，action触发真正计算。内部，DataSet代表一个逻辑计划用来描述：“计算产生的结果”
  * 当action被调用，Spark's query optimizer将会优化逻辑计划并产生一个物理计划供有效的并行分布式执行。
  * 如果想研究逻辑计划和优化过的物理计划的话，参见DataSet的explain函数
  * <br><br>
  *
  *
  ...
  * <br>1：使用SparkSession的read函数<br>
  * 2：根据已有的DataSet创建DataSet
  * <br><br>
  * The most common way is by pointing Spark
  ...
  * @since 1.6.0
  *        <br><br>简而言之DataSet就是：<br>
  *        DataSet是一个强类型的“domain-specific objects”集合，可以在其上使用函数或相关操作。
  *        <br>每一个DataSet都有一个无类型的视图，叫做DataFrame，DataFrame其实就是DataSet[Row]的别名
  */
