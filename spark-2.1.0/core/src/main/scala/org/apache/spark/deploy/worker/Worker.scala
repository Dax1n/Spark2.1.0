...
private[deploy] class Worker(
  ...
  /**
    * registrationRetryTimer初始值为None
    */
  ...
  /**
    ...
    * 由于网络或者master发生变化导致worker重新注册master，重连接次数超过阈值的话退出
    *
    */
  ...
  private def registerWithMaster() {
    ...
    registrationRetryTimer match {//registrationRetryTimer初始值是None,所以第一次连接时候也调用此
      case None =>
        registered = false
        registerMasterFutures = tryRegisterAllMasters()
        ...
        registrationRetryTimer = Some(forwordMessageScheduler.scheduleAtFixedRate(
          new Runnable {
            override def run(): Unit = Utils.tryLogNonFatalError {
              Option(self).foreach(_.send(ReregisterWithMaster))//给自己发消息告诉从新注册Worker，并没有直接给Master发消息
            }
          },
  ...
  private def handleRegisterResponse(msg: RegisterWorkerResponse): Unit = synchronized {
    msg match {
      ...
      case RegisterWorkerFailed(message) =>   //Worker注册失败
        if (!registered) {
