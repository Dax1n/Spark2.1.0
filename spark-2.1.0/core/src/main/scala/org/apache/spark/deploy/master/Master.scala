...
/**
  *
  * 类定义：class Master extends ThreadSafeRpcEndpoint
  *
  * @param rpcEnv
  * @param address
  * @param webUiPort
...
private[deploy] class Master(
  ...
  override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
    ...
    id, workerHost, workerPort, workerRef, cores, memory, workerWebUiUrl) =>
      ...
      if (state == RecoveryState.STANDBY) {
        context.reply(MasterInStandby) //回复消息发送者（worker）当前master是standby状态
      } else if (idToWorker.contains(id)) {
        context.reply(RegisterWorkerFailed("Duplicate worker ID")) //重复注册了
      } else {
  ...
  private def scheduleExecutorsOnWorkers(//TODO scheduleExecutorsOnWorkers返回为每个Worker上分配的cores的数组
    ...
    /**
      * 每一个worker上分配的核心数
      */
    val assignedCores = new Array[Int](numUsable) // Number of cores to give to each worker  每一个worker上的核心数
    /**
      * 每一个worker上新的executor的数目
      */
    val assignedExecutors = new Array[Int](numUsable) // Number of new executors on each worker，

    /**
      * 求当前所有可用worker核心数的和与app需要的核心数的最小值
      */
    ...
    def canLaunchExecutor(pos: Int): Boolean = {
      ...
      //没有限制一个worker一个executor的话或者当前worker给app分配0个executor（就是没分配executor）的话，可以启动新的executor
  ...
  /**
    * Schedule and launch executors on workers<br><br>
    * 调度，在Worker上启动executor
    */
  ...
  private def schedule(): Unit = {
    ...
    //shuffle是为了随机分配worker，要不然容易聚堆,负载均衡
  ...
  private def registerWorker(worker: WorkerInfo): Boolean = {
    ...
    //相同节点之上可能存在id不同的死亡worker，过滤掉死亡的worker
    workers.filter { w =>
