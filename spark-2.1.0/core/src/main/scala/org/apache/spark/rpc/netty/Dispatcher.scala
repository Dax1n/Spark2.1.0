...
/**
  * A message dispatcher, responsible for routing RPC messages to the appropriate endpoint(s).<br>
  * 消息调度收发器，负责路由RPC消息到适当的endpoint
  */
private[netty] class Dispatcher(nettyEnv: NettyRpcEnv) extends Logging {

  /**
    * EndpointData是Endpoint的一个封装
    *
    * @param name
    * @param endpoint
    * @param ref
  ...
  def registerRpcEndpoint(name: String, endpoint: RpcEndpoint): NettyRpcEndpointRef = {
    ...
    synchronized {
      ...
      val data = endpoints.get(name) //类型：EndpointData
      ...
      try {
        while (true) {
          try {
            ...
            //TODO data类型为EndPointData，获取EndPointData的inbox处理其中消息
