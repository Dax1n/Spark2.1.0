...
/**
  ...
  * transforming existing DStreams using operations such as `map`, `window` and `reduceByKeyAndWindow`.
  * <br><br>DStream是Spark Streaming中的基础抽象，表示一个连续的RDD，DStream创建可以有live data或者转换算子创建<br><br>
  *
  * While a Spark Streaming program is running, each DStream
  * periodically generates a RDD, either from live data or by transforming the RDD generated by a
  * parent DStream.<br><br>当Spark Streaming程序运行时候，周期性的由live data或父DStream产生RDD<br><br>
  *
  ...
  * <br><br>DStream包含一些算子函数<br><br><br>
  ...
  */
/**
  * Spark RDD运行作业时候，是在RDD中调用SparkContext的runJob触发作业，在DStream中同样是用SparkContext的runJob触发作业
  *
  *
  */
abstract class DStream[T: ClassTag](
  ...
  /**
    * generatedRDDS是DStream的一个成员变量HashMap，其中key是时间片段，value是RDD
    */
  @transient
  ...
  /** Return the StreamingContext associated with this DStream
    * <br>和Spark RDD类似，RDD中持有SparkContext的引用
    */
  ...
  /**
    * 验证StreamingContext状态，只有StreamingContext状态为INITIALIZED时候可以创建DStream
    */
  ...
  /** Return a new DStream by applying a function to all elements of this DStream. <br><br>
    * 和Spark批处理的RDD类似，返回一个对应表示直接返回，并没有触发计算MappedDStream
    *
    */
  ...
  /**
    ...
    * of this DStream.
    * <br><br>在RDD中count本应该是一个action算子触发job到集群执行，但是在DStream中count不再是一个算子，<br><br>也是一个transform算子，直接生成一个DStream返回表示
    */
